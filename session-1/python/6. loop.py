...
temp =[22, 30, 25, 29, 33, 35, 33, 40]
threshold = 33

for i in temp:
    if i > threshold:
        print(f"Warning! The temperature is {i} degree celcius.")
...
for product, amounts in products.items():
    if amounts["stock"] < amounts["min_required"]:
        restock = amounts["min_required"] - amounts["stock"]
        if restock == 1:
            print(f"Stock alert! Need to restock minimum 1 {product}.")
        else:
            print(f"Stock alert! Need to restock minimum {restock} {product}s.")
