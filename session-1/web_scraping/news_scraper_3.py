...
# Suppress asyncio-related logging
...
if __name__ == "__main__":
    ...
    # if conn is not None:
        try:
            ...
            # print(main_response)
            main_response.html.render()
            # time.sleep(3)
            all_news_links = main_response.html.find('a.title-link')
            # print(all_news_links)
            for news_link in all_news_links:
                link = news_link.attrs['href']
                # print(link)
            # url = "https://www.prothomalo.com/bangladesh/capital/t90c9yp8zp"
                publisher_website, publisher, title, reporter, news_datetime, category, news_body, images = single_news_scraper(link)
                print(f"\nNews obtained from {link}")
